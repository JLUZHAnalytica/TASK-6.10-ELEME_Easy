...
area_data = {
    '北京': ['北京市', '朝阳区', '海淀区', '通州区', '房山区', '丰台区', '昌平区', '大兴区', '顺义区', '西城区', '延庆县', '石景山区', '宣武区', '怀柔区', '崇文区', '密云县', '东城区', '门头沟区', '平谷区'],
    ...
    '重庆': ['重庆市', '江北区', '渝北区', '沙坪坝区', '九龙坡区', '万州区', '永川市', '南岸区', '酉阳县', '北碚区', '涪陵区', '秀山县', '巴南区', '渝中区', '石柱县', '忠县', '合川市', '大渡口区', '开县', '长寿区', '荣昌县', '云阳县', '梁平县', '潼南县', '江津市', '彭水县', '璧山县', '綦江县', '大足县', '黔江区', '巫溪县', '巫山县', '垫江县', '丰都县', '武隆县', '万盛区', '铜梁县', '南川市', '奉节县', '双桥区', '城口县'],
    ...
    '新疆': ['新疆', '新疆维吾尔自治区', '乌鲁木齐市', '伊犁州', '昌吉州', '石河子市', '哈密地区', '阿克苏地区', '巴音郭楞州', '喀什地区', '塔城地区', '克拉玛依市', '和田地区', '阿勒泰州', '吐鲁番地区', '阿拉尔市', '博尔塔拉州', '五家渠市', '克孜勒苏州', '图木舒克市'],
...
def parse_restaurants(ori_data):
    name = ori_data['rst']['name']  # 名
    ID = ori_data['rst']['id']  # id
    lianjie = 'https://h5.ele.me/shop/#id='+ID  # 链接
    lei_all = ori_data['rst']['flavors']
    lei = []
    for k in lei_all:
        lei.append(k['name'])
    xiaoliang = ori_data['rst']['business_info'].split(
        ":")[-1].replace('}', '')  # 订单量
    fen = ori_data['rst']['rating']  # 店铺评分
    fencount = ori_data['rst']['rating_count']  # 评分数
    address = ori_data['rst']['address']
    city = address[0:3]
    latitude = ori_data['rst']['latitude']  # 经度
    longitude = ori_data['rst']['longitude']  # 纬度
    ll = []
    ll.append(latitude)
    ll.append(longitude)
    openhour = ori_data['rst']['opening_hours']  # 营业时间
    phone = ori_data['rst']['phone']  # 电话
    # closing_count_down
    yinye = ori_data['rst']['closing_count_down']
    if yinye:
        yyzt = '营业'
    else:
        yyzt = '关闭'
    youhui = ""
    for acti in ori_data['rst']["activities"]:
        youhui += (';' + acti['tips'])
    ...
    for k, v in area_data.items():
        for i in v:
            if city in i:
                shop = {'商铺名': name, '店铺ID': ID, '营业状态': yyzt, '省份': k, '城市': city,
                        '平台分类': lei, '地址': address, '经纬度': ll, '店铺订单量': xiaoliang,
                        '联系方式': phone, '营业时间': openhour, '店铺链接': lianjie, '评分': fen,
                        '评分数': fencount, '店铺优惠': youhui}
                # print(shop)
                res_data.append(shop)
    return res_data


# def parse_foodlist(ori_data):



with oepn("demo_data/batch_shop_demo_full.json",'r') as fd:
    data = json.loads(fd.read())

with open('test_output.josn','w') as fd:
    fd.write(parse_restaurants(data))